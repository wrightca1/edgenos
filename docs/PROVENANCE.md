...
Short version: **two operator-supplied files still block a self-contained release** — the register
replay (`/mnt/flash/fwd4.txt`, 389,809 writes) and the FM6000 microcode (`ucode_l2.raw`,
`ucode_tail.raw`). The Si5338 clock map was eliminated in alpha4 and stays gone.
...
| `ucode_l2.raw`, `ucode_tail.raw` | 710,406 B | FM6000 microcode, from the EOS image | yes (also embedded inside the replay) |
