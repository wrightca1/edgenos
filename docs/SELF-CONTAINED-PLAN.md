...
**91.5% of EOS's replay is eliminated** — 356,557 of 389,809 lines, with the SPICO firmware dropped
for a fibre-only build. 68.4% with SPICO retained for copper.

```
our 20 generators emit          119,894
final replay file               243,152 lines
  still EOS-derived (SPICO in)  123,258
  still EOS-derived (SPICO out)  33,252
```

`fwd4.txt` is the only operator-supplied file; the Si5338 map went in alpha4 and the microcode in
alpha7.

## ★ L2AR: split on the loop boundary

L2AR was the largest remaining block and resisted both earlier techniques. The reason is that it is
**two different things sharing an address range**:

```
25,426 writes BEFORE the loop, in two contiguous runs (~12,900 and ~12,500) -- a bulk table load
 3,684 writes INSIDE the loop, in bursts of 4/8/12 interleaved with the port bring-up
```

- Collapsing to write-once state reached only 4,606 registers.
- Relocating the whole block moved the in-loop bursts too, and unicast forwarding died — OSPF stayed
  up, ARP resolved, ping 100% loss across 14 rounds.
- **Splitting at the loop anchor works**: lift the pre-loop bulk, leave every in-loop write exactly
  where the trace put it. 3 cold boots, routes 34–35, 17 of 18 ping rounds at zero loss.

That is a fourth move, alongside the three below — and the one that unlocked the block that looked
hardest.

| kind | example | move |
|---|---|---|
| table the pipeline reads | SAF, CM, L2L | collapse to end state |
| table + control strobes | FFU | collapse the table, leave the strobes |
...
| | writes | note |
|---|---:|---|
| MAPPER / FFU / PARSER control | ~17,400 | strobes and sequences left in place deliberately |
| SBus lane tuning (JSS) | 3,719 | board measurement, not MMIO |
| L2AR in-loop bursts | 3,684 | interleaved with port bring-up; moving them breaks unicast |
| MOD / L3AR / misc tails | ~8,400 | small control remainders |

Everything cheap is done. What is left is control writes that are load-bearing exactly where they
sit, and the honest position is that they may not be liftable at all without reimplementing the
bring-up logic rather than relocating its trace.
