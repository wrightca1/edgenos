...
WHAT THE PROGRAM MUST SATISFY, all established in docs/EDGENOS-7150.md (was PARSER-CONVENTIONS):
  ...
  * The frame layout is a property of the INGRESS PORT: front-panel ports carry
    untagged Ethernet (PARSER_INIT_STATE 0x61c70000), the CPU port carries an
    8-byte F64 ISL tag after the SMAC (0x6b3f0000). STATE8[3] discriminates.
...
position at byte offset B is parsed by slice B//4. Datasheet 5.5: the first byte
received occupies the MOST significant byte, so Halfword0 = {byte0,byte1} is
key[31:16] and Halfword1 is key[15:0].

VALIDATED ON HARDWARE 2026-08-10. Cold booted from an image with
fm6000_parserinit removed entirely: routes 34 held for three minutes with ARP
resolved, and no EOS parser table present in the image or the replay.

SCOPE. Ethernet, 0-2 VLAN tags, IPv4 (guarded on IHL=5), IPv6 (guarded on
extension headers), ARP, and the F64 ISL tag. IPv4/OSPF is hardware-proven;
IPv6 and VLAN are verified only against EOS in the simulator and have never
crossed the wire.
"""
import argparse
import sys
...
# SCENARIO_CAM. On this box they are:
#
#   0 Unbound0, 2 ISL_Type0, 3 ISL_Type1, 4 ISL_FType0, 8 IsIPv4, 9 IsIPv6,
#   13 TTL_Expired, 14 HeadFrag, 16 L3_Options, 19 L3_Mcst(DIP),
#   27/28 HdrOffsets[3:4], and 40/42/43 which are mapper-set and beyond the
#   parser's 38-bit SetFlags.
#
# A flag in that list that we leave clear changes the scenario value, which
# selects different FFU rules -- the exact shape of "frames arrive, nothing
# classifies them". This retires an earlier WRONG conclusion that the ISL flags
...
S_ISL_W2 = 0x36      # consumed the tag's first half; second half next
S_DONE_OTHER = 0x34
...
STATE_NAME = {
    ...
    S_ISL_W2: "isl-tag-w2", S_DONE_OTHER: "other-l3",
}

MAX_TAGS = 2

# ★ CPU-injected frames carry an 8-byte F64 ISL tag between the SMAC and the
# EtherType (asic/fm6000/fm6000_txinline.c):
#
#     DMAC     SMAC     <---- F64 tag, 8 bytes ---->     ethertype
#
# Frames from the wire do NOT. Parsing only the untagged layout meant every
# frame the switch transmitted was mis-parsed and dropped -- we received fine
# and never transmitted, so no OSPF adjacency and no ARP. See
# docs/EDGENOS-7150.md (was PARSER-CONVENTIONS).
#
# The tag shifts everything after it by 8 bytes = 2 slices, so it is modelled as
# an offset alongside VLAN tag depth rather than as a separate program. STATE8[1]
# carries both: tag depth in bits 0-1, the ISL flag in bit 2, so the two paths
# never collide even where they share a slice.
ISL_TAG_W0 = 0x0100          # first halfword of EOS's F64 tag; we build the same
...
#
#   Et1 is logical port 40 (docs/EDGENOS-7150.md (was GLORT-MAPPING)), init 0x61c70000
...
def build_program():
    ...
    # assumption recorded in docs/EDGENOS-7150.md (was PARSER-CONVENTIONS). PARSER_INIT_STATE is
    ...
    for isl, depth in [(i, d) for i in (0, 1) for d in range(MAX_TAGS + 1)]:
        ...
        # HeadFrag (scenario bit 6): the head fragment, i.e. fragment offset 0 --
...
def reachable_slices(rule):
    ...
    base = {S_START: 0, S_DMAC_LO: 1, S_SMAC_LO: 2, S_ETYPE: 3, S_TAGGED: 3,
            ...
            # ⚠ base 2, not 4. reachable_slices() adds ISL_SLICES for isl=1, but
            # this state CONSUMES the tag's second half (bytes 16-19 = slice 4)
            # rather than following it -- the +2 shift applies only to states
            # after the tag. 2 + 2 = 4. Getting this wrong put the rule at slice
            # 6 and silently broke the whole ISL path.
            S_ISL_W2: 2}
    if rule.state not in base:
        return []
    return [base[rule.state] + rule.tag_depth + ISL_SLICES * rule.isl]
...
C_TEMPLATE = r"""/* fm6000_parserinit.c - the FM6000 parser program, GENERATED BY US.
 *
 * GENERATED by asic/fm6000/tools/parser_program.py --c -- do not edit.
 *
 * This file replaces a transcription. It previously held %(eos)d address/value
 * pairs copied verbatim from EOS's parser tables (docs/EDGENOS-7150.md (was PROVENANCE) 2.5);
 * it now holds %(ours)d writes emitted from a declarative protocol description
 * in parser_program.py. Nothing here is derived from EOS.
 *
 * The program: Ethernet, 0-2 VLAN tags, IPv4 (guarded on IHL=5), IPv6 (guarded
 * on extension headers), ARP, and the F64 ISL tag on the CPU port. Frame layout
 * is selected by the ingress port's PARSER_INIT_STATE, which the replay
 * programs -- 0x61 in STATE8[3] for the front-panel ports (untagged) and 0x6b
 * for the CPU port (tagged).
 *
 * ⚠ -a DELIBERATELY LISTS THE WHOLE PARSER REGION, not just the addresses we
 * write. The boot script's gen_list_early uses -a to decide which replay lines
 * to drop, and EOS's program occupies far more entries than ours. Listing only
 * our own addresses would leave EOS's remaining entries live in the CAM
 * alongside ours -- and since the CAM resolves to the LAST matching entry, they
 * would win.
 *
 * Cold-boot validated 2026-08-10: routes 34 held for three minutes with
 * fm6000_parserinit absent from the image entirely.
 *
...
#define PARSER_BASE   0x%(base)06x
#define SLICE_STRIDE  0x%(stride)04x
#define RAM_OFFSET    0x%(ram)04x
#define NUM_SLICES    %(slices)d
#define ENTRIES       %(entries)d
...
int main(int argc, char **argv)
{
	...
	if (list) {
		/* the whole region -- see the note above */
		for (s = 0; s < NUM_SLICES; s++)
			for (e = 0; e < ENTRIES; e++)
				for (w = 0; w < 4; w++) {
					printf("%%08x\n", PARSER_BASE + SLICE_STRIDE*s + 4*e + w);
					printf("%%08x\n", PARSER_BASE + SLICE_STRIDE*s + RAM_OFFSET + 4*e + w);
				}
		return 0;
	}
	if (!dry) {
	...
	if (!dry)
		printf("fm6000_parserinit: %%d writes (ours)\n",
...
def emit_c(out_path):
    ws = writes(place(build_program()))
    rows = "".join(f"\t{{0x{a:08x},0x{v:08x}}},\n" for a, v in ws)
    src = C_TEMPLATE % dict(eos=16960, ours=len(ws), base=PARSER_BASE,
                            stride=SLICE_STRIDE, ram=RAM_OFFSET,
                            slices=NUM_SLICES, entries=ENTRIES_PER_SLICE,
                            rows=rows)
    with open(out_path, "w") as fh:
        fh.write(src)
    print(f"wrote {out_path}: {len(ws)} writes")
    return 0
...
def main():
    ...
    ap.add_argument("--c", metavar="FILE", dest="c_out",
                    help="emit a drop-in fm6000_parserinit.c from our program")
    ...
    if not (args.emit or args.check or args.summary or args.addresses
            or args.splice or args.c_out):
        ap.print_help()
    return 0
