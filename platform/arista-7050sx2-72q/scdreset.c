...
/* THE CHASSIS STATUS LEDs.
 *
 * System status, the two PSU lamps, the fan lamp and the blue locator beacon.
 * They are NOT on the SCD -- the whole SCD block above the port LEDs reads zero
 * on a healthy running EOS, with the status lamp lit, and a lit LED cannot be
 * driven from an all-zero register. They are one-byte registers on the same
 * StandbyCpld as the fans, at 0x60 on SMBus accelerator 0 bus 0, which is where
 * Arista's own arista/platforms/cpu/crow.py puts CrowFanCpld.
 *
 * Their offsets come from the generated map, not from this file -- see the note
 * on the include below. What IS here is the encoding, which was established by
 * looking at the panel rather than inferred:
 *
 *   POLARITY IS ACTIVE HIGH. Green is one bit, red is another, both is amber,
 *   and a set bit lights the lamp. That is the OPPOSITE of the fan tray LEDs at
 *   0x24/0x25 on this same part, which Arista's crow-fan-driver.c drives active
 *   low -- one CPLD, two conventions, and assuming it had only one would have
 *   got this backwards. CROW_LED_ACTIVE_LOW=1 flips it without an edit.
 *
 *   STATUS AND THE BEACON ARE ONE PHYSICAL LED. Green set and the beacon on
 *   shows blue: the blue bit wins. The beacon is not a lamp lit alongside a
 *   status colour, it overrides it -- which is what you want from a locator,
 *   and why the health policy below never touches it. An operator lighting the
 *   beacon to find the box in a rack should not have a health loop switching it
 *   off underneath them.
 *
 * Every write is read-modify-write on the two colour bits only. The other six
 * bits of these registers are undescribed, and this is the CPLD that has
 * powered the box off twice when swept, so nothing here touches a bit it cannot
 * name.
 */
/* THE FIVE OFFSETS ARE NOT IN THIS FILE, ON PURPOSE.
 *
 * Everything above is mechanism and is ours. The addresses are not: they come
 * out of the board description that ships on the switch, which is vendor
 * material, and this platform's rule is that the mechanism is published and the
 * vendor's numbers are regenerated by the operator on their own machine. The
 * SCD offsets and the fan registers stay hardcoded because Arista genuinely
 * publishes those; these five are the ones that are only in the .fdl.
 *
 *   tools/mkstatusleds.sh <switch-ip> > tools/crow-statusled-map.h
 *
 * Without that header the code still builds and every other command works; the
 * status LED commands say what to run instead of failing obscurely.
 */
#if defined(__has_include)
#  if __has_include("crow-statusled-map.h")
#    include "crow-statusled-map.h"
#  endif
#endif

#ifndef CROW_STATUSLED_MAP_PRESENT
#define CROW_STATUSLED_MAP_PRESENT 0
#define CROW_LED_STATUS  0u
#define CROW_LED_BEACON  0u
#define CROW_LED_PSU1    0u
#define CROW_LED_PSU2    0u
#define CROW_LED_FAN     0u
#define LED_GREEN_BIT    1
#define LED_RED_BIT      0
#define LED_BLUE_BIT     0
#endif

static int statusled_map(void)
{
    if (CROW_STATUSLED_MAP_PRESENT) return 1;
    fprintf(stderr,
            "statusled: no LED map built in. These offsets are vendor board\n"
            "           description data, so they are generated, not shipped:\n"
            "             tools/mkstatusleds.sh <switch-ip> > tools/crow-statusled-map.h\n"
            "           then rebuild.\n");
    return 0;
}

enum { SL_OFF = 0, SL_GREEN, SL_RED, SL_AMBER };

static int led_active_high(void)
{
    const char *e = getenv("CROW_LED_ACTIVE_LOW");
    return !(e && *e && *e != '0');
}

static const struct { const char *name; unsigned reg; } status_leds[] = {
    { "status", CROW_LED_STATUS },
    { "psu1",   CROW_LED_PSU1   },
    { "psu2",   CROW_LED_PSU2   },
    { "fan",    CROW_LED_FAN    },
};
#define N_STATUS_LEDS ((int)(sizeof(status_leds) / sizeof(status_leds[0])))

static const char *color_name(int c)
{
    return c == SL_GREEN ? "green" : c == SL_RED   ? "red"
         : c == SL_AMBER ? "amber" : "off";
}

static int color_parse(const char *s)
{
    if (!strcmp(s, "off"))   return SL_OFF;
    if (!strcmp(s, "green")) return SL_GREEN;
    if (!strcmp(s, "red"))   return SL_RED;
    if (!strcmp(s, "amber") || !strcmp(s, "orange")) return SL_AMBER;
    return -1;
}

/* Decode one register into a colour. Returns -1 if the CPLD would not answer,
 * which the caller must not confuse with "off". */
static int status_led_get(unsigned reg)
{
    int v = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, reg), g, r;

    if (v < 0) return -1;
    g = (v >> LED_GREEN_BIT) & 1;
    r = (v >> LED_RED_BIT) & 1;
    if (!led_active_high()) { g = !g; r = !r; }
    return (g && r) ? SL_AMBER : g ? SL_GREEN : r ? SL_RED : SL_OFF;
}

static int status_led_set(unsigned reg, int color)
{
    int v = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, reg);
    int g = (color == SL_GREEN || color == SL_AMBER);
    int r = (color == SL_RED   || color == SL_AMBER);

    if (v < 0) return -1;
    if (!led_active_high()) { g = !g; r = !r; }
    v = g ? (v |  (1 << LED_GREEN_BIT)) : (v & ~(1 << LED_GREEN_BIT));
    v = r ? (v |  (1 << LED_RED_BIT))   : (v & ~(1 << LED_RED_BIT));
    return smb_wr_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, reg, (unsigned)v & 0xff);
}

static int beacon_get(void)
{
    int v = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, CROW_LED_BEACON);

    if (v < 0) return -1;
    v = (v >> LED_BLUE_BIT) & 1;
    return led_active_high() ? v : !v;
}

static int beacon_set(int on)
{
    int v = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, CROW_LED_BEACON);
    int b = led_active_high() ? on : !on;

    if (v < 0) return -1;
    v = b ? (v | (1 << LED_BLUE_BIT)) : (v & ~(1 << LED_BLUE_BIT));
    return smb_wr_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, CROW_LED_BEACON,
                      (unsigned)v & 0xff);
}

static int do_statusled_show(void)
{
    int i, b;

    if (!statusled_map()) return 1;
    if (crow_probe() < 0) return 1;
    for (i = 0; i < N_STATUS_LEDS; i++) {
        int raw = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, status_leds[i].reg);
        int c   = status_led_get(status_leds[i].reg);
        if (raw < 0 || c < 0) { printf("  %-7s 0x%02x  unreadable\n",
                                       status_leds[i].name, status_leds[i].reg);
                                continue; }
        printf("  %-7s 0x%02x = 0x%02x  %s\n",
               status_leds[i].name, status_leds[i].reg, raw, color_name(c));
    }
    b = beacon_get();
    {
        int raw = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, CROW_LED_BEACON);
        printf("  %-7s 0x%02x = 0x%02x  %s\n", "beacon", CROW_LED_BEACON,
               raw < 0 ? 0 : raw, b < 0 ? "unreadable" : b ? "on" : "off");
    }
    printf("  (%s)\n", led_active_high() ? "active high" : "active low");
    return 0;
}
...
/* WHAT THE CHASSIS LEDs SHOULD SAY.
 *
 * A status LED that is always green is decoration. These are driven from state
 * we actually measure, and anything we cannot measure shows as amber rather
 * than green -- an unread sensor is not a healthy sensor.
 *
 *   fan      green   all four present and turning
 *            amber   a bay is empty (this box ships with four)
 *            red     a fitted fan reads zero rpm, or the CPLD stopped answering
 *   psu1/2   green   presence bit set in SCD 0x5000 (bit 0, bit 1)
 *            off     bay empty -- off, not red, because a one-supply box is a
 *                    legitimate configuration and a permanent red is noise
 *   status   green   fans fine and the hottest sensor below the ceiling
 *            amber   degraded: a fan bay empty, or a sensor unreadable
 *            red     a fan has stopped, or we are at or above TMAX
 *
 * The beacon is never touched here. It is an operator's locator, and a health
 * loop that switched it off would fight the person trying to find the box.
 *
 * PSU presence is bits 0 and 1 of SCD 0x5000, which Arista publishes:
 * arista/platforms/upperlake.py declares GpioDesc("psu1_present", 0x5000, 1)
 * and GpioDesc("psu2_present", 0x5000, 0). Note that board has them the other
 * way round from the order used here -- which of the two bits is which supply
 * is per-board, and getting it backwards mislabels one lamp and nothing else.
 */
static int psu_present(int slot)                  /* slot 1 or 2 */
{
    return (rd(scd, 0x5000u) >> (slot - 1)) & 1;
}

static void health_leds(int hot)
{
    int present, missing = 0, stopped = 0, i, fan_color, status_color;

    if (!CROW_STATUSLED_MAP_PRESENT) return;
    present = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, CROW_PRESENT);

    if (present < 0) {
        /* The CPLD is not answering. Do not paint green over that. */
        status_led_set(CROW_LED_STATUS, SL_RED);
        return;
    }
    for (i = 0; i < CROW_NFAN; i++) {
        if ((present >> i) & 1) { missing++; continue; }   /* presence is active low */
        if (crow_rpm(i) <= 0) stopped++;
    }

    fan_color = stopped ? SL_RED : missing ? SL_AMBER : SL_GREEN;
    status_led_set(CROW_LED_FAN, fan_color);

    status_led_set(CROW_LED_PSU1, psu_present(1) ? SL_GREEN : SL_OFF);
    status_led_set(CROW_LED_PSU2, psu_present(2) ? SL_GREEN : SL_OFF);

    if (stopped || hot >= TH_TMAX)      status_color = SL_RED;
    else if (missing || hot < 0)        status_color = SL_AMBER;
    else                                status_color = SL_GREEN;
    status_led_set(CROW_LED_STATUS, status_color);
}
...
static int do_thermal(int interval, int once)
{
    ...
    printf("thermal: floor %d%%, %dC->%dC maps %d%%->%d%%, every %ds%s\n",
           CROW_MIN_PCT, TH_TMIN, TH_TMAX, CROW_MIN_PCT, TH_MAXPCT, interval,
           CROW_STATUSLED_MAP_PRESENT ? ", status LEDs driven"
                                      : ", no status LED map (see mkstatusleds.sh)");
    fflush(stdout);
    ...
    while (!th_stop) {
        ...
        if (!getenv("SCDRESET_NO_STATUS_LEDS")) health_leds(hot);
    ...
    for (i = 0; i < CROW_NFAN; i++) crow_set_pct(i, TH_MAXPCT, 0);
    /* Fans flat out and the status LED amber: the box is safe but nothing is
     * managing it any more, and the panel should not claim otherwise. */
    if (!getenv("SCDRESET_NO_STATUS_LEDS")) status_led_set(CROW_LED_STATUS, SL_AMBER);
    fflush(stdout);
    return 0;
}
...
/* ONE PETTER, NOT ONE PER CALL.
 *
 * `pet` forks a process that lives forever, and init supervises it with a
 * 30-second loop that just called `pet` again every time. Each call left
 * another immortal process behind: 2,918 of them after a day of uptime, all
 * writing the same watchdog register. Harmless individually, unbounded in
 * aggregate, and the box would eventually run out of pids.
 *
 * The supervisor is not the bug -- restarting a dead petter is exactly what it
 * should do. The bug is that `pet` had no way to say "one is already running",
 * so the fix belongs here rather than in a cleverer shell loop. A pidfile plus
 * a liveness check makes a repeat call a no-op and keeps the supervisor honest.
 */
#define PET_PIDFILE "/tmp/scdreset-pet.pid"

static int pet_running(void)
{
    char path[64], buf[128];
    FILE *f = fopen(PET_PIDFILE, "r");
    long pid = 0;
    int fd, n;
    ...
    /* Alive is not enough -- the pid could have been recycled by anything.
     * Check it is still one of ours before believing it. */
    ...
    if (n <= 0) return 0;
    buf[n] = 0;
    return strstr(buf, "scdreset") ? (int)pid : 0;
}


/* THE WHOLE-CHASSIS LIGHT SHOW.
 *
 * This lives here rather than in leddance because scdreset is the only binary
 * that can reach both halves of the panel: the port LEDs are memory-mapped on
 * the SCD, the chassis lamps are one-byte registers behind the SCD's SMBus
 * master. Duplicating the SMBus accelerator into leddance to avoid one command
 * here would be the worse trade.
 *
 * The port palette was mapped by writing patterns and having someone look:
 *
 *   bit 28   green            bit 27   amber (and amber wins over green)
 *   bit 24   BLINK -- modulates whatever colour is set; alone it shows nothing
 *   bit 26   implemented, no visible effect
 *   bit 25   implemented, no visible effect
 *   bits 29-31 not implemented: writing 0xffffffff reads back 0x1f000000
 *
 * NOTE THAT BLINK IS BIT 24, NOT BIT 25. Arista's scd-led.c defines
 * SCD_BLINK_MASK as BIT(25), and on this SCD generation that bit does nothing
 * while 24 flashes. Taking the driver's constant on trust would have produced a
 * blink option that silently did not blink -- which is why every one of these
 * was written to a register and looked at, rather than read out of a header.
 *
 * The low 0x06ff00 intensity field in scd-led.c's legacy table does not exist
 * here either: write it and it reads back as zero.
 *
 * Port writes are cheap; chassis writes are SMBus transactions costing
 * milliseconds, so they run on their own slower cadence rather than once per
 * frame.
 *
 * ON THE WAY OUT the ports are blanked and the chassis lamps put back to what
 * the health policy says. If the bridge is running, its ledsync will repaint
 * the real link state within five seconds -- and it fights this show while it
 * runs, which is exactly right and is why each frame is rewritten in full.
 */
#define LED_GREEN  0x10000000u
#define LED_AMBER  0x08000000u
#define LED_BLINK  0x01000000u
#define SHOW_PORTS 72

static uint32_t show_reg(int i)          /* 0-based across the whole panel */
{
    return i < 48 ? 0x6100u + 0x10u * (unsigned)i
                  : 0x6400u + 0x10u * (unsigned)(i - 48);
}

static void show_frame(const uint32_t *v)
{
    int i;
    for (i = 0; i < SHOW_PORTS; i++) wr(scd, show_reg(i), v[i]);
}

static void show_blank(void)
{
    uint32_t v[SHOW_PORTS];
    memset(v, 0, sizeof(v));
    show_frame(v);
}

static volatile int show_stop;
static void show_sigint(int s) { (void)s; show_stop = 1; }

static void show_nap(int ms)
{
    struct timespec t = { ms / 1000, (long)(ms % 1000) * 1000000L };
    nanosleep(&t, NULL);
}

/* Chassis lamps, cycled on their own slow clock. Returns the next step. */
static int show_chassis(int step)
{
    static const int cyc[] = { SL_GREEN, SL_AMBER, SL_RED, SL_AMBER };

    if (!CROW_STATUSLED_MAP_PRESENT) return step + 1;
    status_led_set(CROW_LED_STATUS, cyc[step % 4]);
    status_led_set(CROW_LED_PSU1,   cyc[(step + 1) % 4]);
    status_led_set(CROW_LED_PSU2,   cyc[(step + 2) % 4]);
    status_led_set(CROW_LED_FAN,    cyc[(step + 3) % 4]);
    beacon_set(step % 2);
    return step + 1;
}

/* A small deterministic PRNG. rand() would do, but this keeps the show
 * reproducible when someone says "it did something odd about ten seconds in". */
static uint32_t show_rnd_state = 0x1d872b41u;
static uint32_t show_rnd(void)
{
    show_rnd_state ^= show_rnd_state << 13;
    show_rnd_state ^= show_rnd_state >> 17;
    show_rnd_state ^= show_rnd_state << 5;
    return show_rnd_state;
}

/* STYLE 2: nothing that scans left to right.
 *
 *   sparkle   mostly dark, ports lighting and decaying at random
 *   ripple    rings expanding from the middle of the panel, alternating colour
 *   meter     a bar that rises, overshoots into flashing amber, and settles
 *   rain      drops that appear high and fall through the QSFP cages
 */
static void show_style2(uint32_t *v, int phase, int t, int frame)
{
    static uint32_t decay[SHOW_PORTS];
    int i;

    switch (phase) {
    case 0:                                            /* sparkle */
        for (i = 0; i < SHOW_PORTS; i++) {
            if (decay[i]) decay[i]--;
            if ((show_rnd() & 0x3f) == 0) decay[i] = 6 + (show_rnd() & 7);
            v[i] = decay[i] ? ((decay[i] & 4) ? LED_GREEN : LED_AMBER) : 0;
        }
        break;

    case 1: {                                          /* ripple from centre */
        int mid = SHOW_PORTS / 2, r = (t * 3) / 2;
        for (i = 0; i < SHOW_PORTS; i++) {
            int d = i > mid ? i - mid : mid - i;
            int ring = r - d;
            if (ring >= 0 && ring < 12)
                v[i] = (ring / 3) & 1 ? LED_AMBER : LED_GREEN;
        }
        break;
    }

    case 2: {                                          /* bar meter */
        int peak = SHOW_PORTS - 8, lvl;
        lvl = (t < 40) ? (t * peak) / 40
                       : peak + ((t - 40) % 8 < 4 ? 4 : 0);
        if (lvl > SHOW_PORTS - 1) lvl = SHOW_PORTS - 1;
        for (i = 0; i <= lvl; i++) v[i] = LED_GREEN;
        if (lvl >= peak)
            for (i = peak; i <= lvl; i++) v[i] = LED_AMBER | LED_BLINK;
        else
            v[lvl] = LED_AMBER;
        break;
    }
    ...
    default: {                                         /* rain */
        ...
        for (drop = 0; drop < 10; drop++) {
            int lane = (drop * 7 + frame / 40) % 24;
            int pos  = (frame / 2 + drop * 11) % 40;
            if (pos < 24) {
                int idx = 48 + ((lane + pos) % 24);
                v[idx] = LED_GREEN;
                if (pos > 0) v[48 + ((lane + pos - 1) % 24)] = LED_GREEN | LED_BLINK;
            } else {
                int idx = (drop * 5 + pos) % 48;
                v[idx] = LED_AMBER;
            }
        }
        break;
...
static int do_ledshow(int seconds, int style)
{
    uint32_t v[SHOW_PORTS];
    time_t end;
    int chassis_step = 0, frame = 0;

    if (CROW_STATUSLED_MAP_PRESENT && crow_probe() < 0)
        printf("ledshow: no CPLD -- ports only\n");

    signal(SIGINT,  show_sigint);
    signal(SIGTERM, show_sigint);
    printf("ledshow: style %d, %d ports, green and amber, for %ds (Ctrl-C to stop)\n",
           style, SHOW_PORTS, seconds);
    fflush(stdout);

    end = time(NULL) + seconds;
    while (!show_stop && time(NULL) < end) {
        int i, phase, t = frame % 60;

        memset(v, 0, sizeof(v));
        if (style == 2) {
            show_style2(v, (frame / 60) % 4, t, frame);
            show_frame(v);
            if (frame % 12 == 0) chassis_step = show_chassis(chassis_step);
            frame++;
            show_nap(40);
            continue;
        }
        phase = (frame / 60) % 5;
        switch (phase) {
        case 0:                                  /* fill green, drain amber */
            for (i = 0; i < SHOW_PORTS; i++)
                v[i] = (t < 30) ? (i <= t * SHOW_PORTS / 30 ? LED_GREEN : 0)
                                : (i <= (t - 30) * SHOW_PORTS / 30 ? 0 : LED_AMBER);
            break;
        case 1: {                                /* amber head, green tail */
            int head = (t * SHOW_PORTS) / 60;
            for (i = 0; i < SHOW_PORTS; i++) {
                int d = head - i;
                if (d == 0)                 v[i] = LED_AMBER | LED_BLINK;
                else if (d > 0 && d < 8)    v[i] = LED_GREEN;
            }
            break;
        }
        case 2:                                  /* alternating, flipping */
            for (i = 0; i < SHOW_PORTS; i++)
                v[i] = ((i + t / 6) & 1) ? LED_GREEN : LED_AMBER;
            break;
        case 3: {                                /* both ends to the middle */
            int n = (t * (SHOW_PORTS / 2)) / 60;
            for (i = 0; i <= n; i++) {
                v[i] = LED_GREEN;
                v[SHOW_PORTS - 1 - i] = LED_AMBER;
            }
            break;
        }
        default:                                 /* everything, flashing */
            for (i = 0; i < SHOW_PORTS; i++)
                v[i] = ((i / 6) & 1 ? LED_AMBER : LED_GREEN) | LED_BLINK;
            break;
        }
        show_frame(v);
    ...
    printf("ledshow: done, restoring\n");
    show_blank();
    if (CROW_STATUSLED_MAP_PRESENT) {
        beacon_set(0);
        health_leds(temp_hottest(NULL, NULL));
    }
    fflush(stdout);
    return 0;
}
...
static void usage(void)
{
    printf("usage:\n"
           ...
           "  scdreset statusled [<led> <colour>] chassis status/psu/fan LEDs\n"
           "  scdreset statusled beacon <on|off>  the blue locator beacon\n"
           "  scdreset crowreg <reg> [val]        one byte on the fan/status CPLD\n"
           "  scdreset ledshow [secs] [1|2]       whole-chassis light show\n"
           "        1 = scans and fills   2 = sparkle, ripple, meter, rain\n"
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(cmd, "pet") && argc > 2) {
        uint32_t v = wd_value((unsigned)strtoul(argv[2], NULL, 0));
        int already = pet_running();
        pid_t pid;
        if (already && !getenv("SCDRESET_PET_FORCE")) {
            printf("already petting in pid %d (SCDRESET_PET_FORCE=1 to add another)\n",
                   already);
            return 0;
        }
        if (!(scd = map_res(SCD_RES, SCD_MAP_SIZE, 1))) return 1;
    ...
    if (!strcmp(cmd, "statusled")) {
        int i, c;
        if (!statusled_map()) return 1;
        if (!(scd = map_res(SCD_RES, SCD_MAP_SIZE, 1))) return 1;
        if (argc < 3) return do_statusled_show();
        if (!strcmp(argv[2], "beacon")) {
            if (argc < 4) { printf("usage: scdreset statusled beacon <on|off>\n");
                            return 2; }
            if (crow_probe() < 0) return 1;
            if (beacon_set(!strcmp(argv[3], "on")) < 0) {
                fprintf(stderr, "statusled: beacon write failed\n"); return 1; }
            return do_statusled_show();
        }
        if (argc < 4) {
            printf("usage: scdreset statusled                       show all\n");
            printf("       scdreset statusled <name> <colour>\n");
            printf("       scdreset statusled beacon <on|off>\n");
            printf("  names   status psu1 psu2 fan all beacon\n");
            printf("  colours off green red amber\n");
            return 2;
        }
        if ((c = color_parse(argv[3])) < 0) {
            fprintf(stderr, "statusled: unknown colour '%s'\n", argv[3]); return 2; }
        if (crow_probe() < 0) return 1;
        if (!strcmp(argv[2], "all")) {
            int bad = 0;
            for (i = 0; i < N_STATUS_LEDS; i++)
                if (status_led_set(status_leds[i].reg, c) < 0) bad++;
            if (bad) fprintf(stderr, "statusled: %d of %d writes failed\n",
                             bad, N_STATUS_LEDS);
            return do_statusled_show();
        }
        for (i = 0; i < N_STATUS_LEDS; i++)
            if (!strcmp(argv[2], status_leds[i].name)) {
                if (status_led_set(status_leds[i].reg, c) < 0) {
                    fprintf(stderr, "statusled: write failed\n"); return 1; }
                return do_statusled_show();
            }
        fprintf(stderr, "statusled: unknown led '%s'\n", argv[2]);
        return 2;
    }
    /* Raw single-register access to the fan/status CPLD. ONE register, named on
     * the command line -- there is deliberately no range form, because sweeping
     * this part has powered the box off twice. */
    if (!strcmp(cmd, "ledshow")) {
        int secs  = argc > 2 ? atoi(argv[2]) : 60;
        int style = argc > 3 ? atoi(argv[3]) : 1;
        if (secs < 1) secs = 1;
        if (style < 1 || style > 2) style = 1;
        if (!(scd = map_res(SCD_RES, SCD_MAP_SIZE, 1))) return 1;
        return do_ledshow(secs, style);
    }
    if (!strcmp(cmd, "crowreg")) {
        unsigned reg;
        int v;
        if (argc < 3) { printf("usage: scdreset crowreg <reg> [value]\n"); return 2; }
        reg = strtoul(argv[2], NULL, 0) & 0xff;
        if (!(scd = map_res(SCD_RES, SCD_MAP_SIZE, 1))) return 1;
        if (crow_probe() < 0) return 1;
        if (argc > 3) {
            unsigned val = strtoul(argv[3], NULL, 0) & 0xff;
            if (smb_wr_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, reg, val) < 0) {
                fprintf(stderr, "crowreg: write failed\n"); return 1; }
        }
        v = smb_rd_reg(CROW_ACCEL, CROW_BUS, CROW_ADDR, reg);
        if (v < 0) { fprintf(stderr, "crowreg: read failed\n"); return 1; }
        printf("crow 0x%02x reg 0x%02x = 0x%02x\n", CROW_ADDR, reg, v);
        return 0;
    }
